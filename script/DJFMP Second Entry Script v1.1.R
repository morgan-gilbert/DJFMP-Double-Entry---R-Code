...
if (unique_output==TRUE) {
        first_orphan_file <- paste("./Output/",start_time,"first_orphan.csv", sep="")
...
if (unique_output==TRUE) {
        second_orphan_file <- paste("./Output/",start_time,"second_orphan.csv", sep="")
...
first_site_date_table$SampleID<-NULL
second_site_date_table$SampleID<-NULL
...
if (unique_output == TRUE){
        sample_comparison_file <- paste("./Output/",start_time,"sample_comparison_results.html", sep="")
...
if (unique_output==TRUE){
        catch_comparison_file <- paste("./Output/",start_time,"catch_comparison_results.html", sep="")
        cat(catch_output$html_output, file=catch_comparison_file)
}


# Site Detail Comparison --------------------------------------------------
site_output<-compare_df(first_site_date_table, second_site_date_table, group_col=c("ID"), limit_html = 100000, tolerance = 1, tolerance_type = "difference", exclude = c("Entry", "StationCode","SampleDate", "SampleTime"), keep_unchanged_cols= TRUE, stop_on_error = FALSE)
...
if (unique_output==TRUE){
        site_comparison_file <- paste("./Output/",start_time,"site_comparison_results.html", sep="")
...
if (unique_output==TRUE) {
        catch_diff_file <- paste("./Output/",start_time,"catch_diff_summary.csv", sep="")
